...
export function runCollisionSystem() {
  ...
  projectiles.forEach(proj => {
    ...
    if (isPlayerBullet) {
      // 1. Kiểm tra va chạm đạn của Player trúng Siêu Boss
      if (activeBossInstance && activeBossInstance.hp > 0 && activeBossInstance.state !== 'death') {
        ...
        // Kiểm tra đạn (hình tròn) với Boss (hình chữ nhật)
        ...
        if (distanceSquared < (proj.radius * proj.radius)) {
           // Bỏ qua nếu đạn xuyên đã từng trúng boss
    ...
    if (distanceSquared < (player.radius * player.radius)) {
      if (!isInvincible) {
        ...
        updatePlayer({
          hp: nextHp,
          shield: nextShield,
          knockbackVx: Math.cos(pushAngle) * 15, // Bị văng mạnh
  ...
  if (portal && portal.active) {
    ...
    if (dist <= player.radius + portal.radius) {
      // Dịch chuyển thắng game hoặc sang tầng tiếp theo
